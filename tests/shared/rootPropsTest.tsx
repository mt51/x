...
export default function rootPropsTest(
  ...
  const Component = require(`../../src/${componentName}`).default as any;
  ...
  describe(`RootProps${name}`, () => {
    ...
    it(
      ['rootClassName', subComponentName].filter((v) => v).join(' '),
      async () => {
          ...
          return (
            <TriggerMockContext.Provider value={triggerContext}>
              <div id="holder" className="holder" ref={holderRef}>
                {show && (
                  <XProvider getPopupContainer={() => holderRef.current!}>
                    {node}
                  </XProvider>
                )}
              </div>
