...
describe('xFetch', () => {
  ...
  it('should call middlewares', async () => {
    ...
    const middlewares = {
      ...
      onResponse: jest.fn(
        async () => new Response('{"data": "modified"}', { status: 200 }),
      ),
    };
  ...
  it('should throw an error on non-200 status', async () => {
    ...
    await expect(xFetch(baseURL)).rejects.toThrow(
  ...
  it('should throw an error if fetch is not a function', async () => {
    await expect(
      xFetch(baseURL, { fetch: 'not_a_function' as any }),
    ).rejects.toThrow('The options.fetch must be a typeof fetch function!');
  });
