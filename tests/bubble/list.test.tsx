...
import type { BubbleListRef } from '../../src/bubble/BubbleList';
...
describe('Bubble.List', () => {
  ...
  it('not autoScroll if user scroll', () => {
    ...
    const listDOM =
      container.querySelector<HTMLDivElement>('.ant-bubble-list')!;
  ...
  it('roles function', () => {
    ...
    expect(
      container.querySelector('.ant-bubble-end .ant-bubble-avatar > span')!,
    ).toHaveStyle({
      background: '#FF0000',
    });
  });
  ...
  it('autoScroll when data change', () => {
    const scrollSpy = spyElementPrototypes(HTMLElement, {
      ...
      getBoundingClientRect() {
        if (
          (this as any as HTMLElement).classList.contains('ant-bubble-list')
        ) {
          return {
            top: 0,
            bottom: 10,
