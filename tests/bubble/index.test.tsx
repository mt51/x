...
describe('bubble', () => {
    ...
    const { container } = render(
      <Bubble
        ...
        messageRender={(content) => (
          <span className="test-messageRender">{content}</span>
        )}
      />,
    );
    const element = container.querySelector<HTMLSpanElement>(
      '.ant-bubble .test-messageRender',
    );
    expect(element).toBeTruthy();
  ...
  it('reset content if changed', async () => {
    ...
    expect(
      container.querySelector<HTMLDivElement>('.ant-bubble-content')!
        .textContent,
    ).toEqual('b');
  });
});
